# Allocator Benchmark Audit — improved_phase2_aggressive_neutral_cash_unlock
...
candidate:improved_phase2_aggressive_neutral_cash_unlock      0.0739   0.0786  0.9402       -0.1250 -0.0260  0.5909        0.0629         0.0008
```

## Risk Contribution — improved_phase2_aggressive_neutral_cash_unlock

```
            sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
       taa_10m_sma             0.1152                 0.0000                 0.5602                        0.4450
dual_momentum_topn             0.0925                 0.0000                 0.4398                        0.3474
```

**Hidden concentration flagged:**
- `taa_10m_sma`: dollar weight 11.5% but risk contribution 56.0% (delta +44.5pp).
- `dual_momentum_topn`: dollar weight 9.2% but risk contribution 44.0% (delta +34.7pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +7.39% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.940 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +7.39% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.940 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +7.39% vs HRP (internal) +4.19%); on Sharpe? **YES** (cand 0.940 vs HRP (internal) 0.925).
- Does the candidate beat **production** on annualised return? **YES** (cand +7.39% vs prod +6.89%); on Sharpe? **YES**.
