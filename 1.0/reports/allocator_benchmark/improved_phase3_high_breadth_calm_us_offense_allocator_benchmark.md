# Allocator Benchmark Audit — improved_phase3_high_breadth_calm_us_offense
...
candidate:improved_phase3_high_breadth_calm_us_offense      0.0727   0.0752  0.9664       -0.1190 -0.0248  0.6107        0.0665         0.0009
```

## Risk Contribution — improved_phase3_high_breadth_calm_us_offense

```
            sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
       taa_10m_sma             0.1091                 0.0000                 0.5395                        0.4305
dual_momentum_topn             0.0941                 0.0000                 0.4605                        0.3663
```

**Hidden concentration flagged:**
- `taa_10m_sma`: dollar weight 10.9% but risk contribution 54.0% (delta +43.0pp).
- `dual_momentum_topn`: dollar weight 9.4% but risk contribution 46.0% (delta +36.6pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +7.27% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.966 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +7.27% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.966 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +7.27% vs HRP (internal) +4.19%); on Sharpe? **YES** (cand 0.966 vs HRP (internal) 0.925).
- Does the candidate beat **production** on annualised return? **YES** (cand +7.27% vs prod +6.89%); on Sharpe? **YES**.
