# Allocator Benchmark Audit — improved_phase4_sector_20pct_offense
...
production:improved_phase2b_regime_confidence_boost      0.0689   0.0779  0.8848       -0.1398 -0.0262  0.4932        0.0562         0.0007
     candidate:improved_phase4_sector_20pct_offense      0.0764   0.0822  0.9306       -0.1433 -0.0268  0.5335        0.0782         0.0010
```

## Risk Contribution — improved_phase4_sector_20pct_offense

```
            sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
dual_momentum_topn             0.0841                 0.0000                 0.5038                        0.4197
       taa_10m_sma             0.0839                 0.0000                 0.4962                        0.4123
```

**Hidden concentration flagged:**
- `dual_momentum_topn`: dollar weight 8.4% but risk contribution 50.4% (delta +42.0pp).
- `taa_10m_sma`: dollar weight 8.4% but risk contribution 49.6% (delta +41.2pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +7.64% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.931 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +7.64% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.931 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +7.64% vs HRP (internal) +4.19%); on Sharpe? **YES** (cand 0.931 vs HRP (internal) 0.925).
- Does the candidate beat **production** on annualised return? **YES** (cand +7.64% vs prod +6.89%); on Sharpe? **YES**.
