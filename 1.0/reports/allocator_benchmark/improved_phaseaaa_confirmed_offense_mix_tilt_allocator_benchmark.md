# Allocator Benchmark Audit — improved_phaseaaa_confirmed_offense_mix_tilt
...
candidate:improved_phaseaaa_confirmed_offense_mix_tilt      0.0711   0.0759  0.9360       -0.1175 -0.0252  0.6049        0.0608         0.0008
```

## Risk Contribution — improved_phaseaaa_confirmed_offense_mix_tilt

```
            sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
       taa_10m_sma             0.1132                 0.0000                 0.5425                        0.4293
dual_momentum_topn             0.0967                 0.0000                 0.4575                        0.3608
```

**Hidden concentration flagged:**
- `taa_10m_sma`: dollar weight 11.3% but risk contribution 54.2% (delta +42.9pp).
- `dual_momentum_topn`: dollar weight 9.7% but risk contribution 45.8% (delta +36.1pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +7.11% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.936 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +7.11% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.936 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +7.11% vs HRP (internal) +4.19%); on Sharpe? **YES** (cand 0.936 vs HRP (internal) 0.925).
- Does the candidate beat **production** on annualised return? **YES** (cand +7.11% vs prod +6.89%); on Sharpe? **YES**.
