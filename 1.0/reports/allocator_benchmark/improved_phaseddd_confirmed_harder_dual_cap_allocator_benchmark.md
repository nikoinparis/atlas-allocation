# Allocator Benchmark Audit — improved_phaseddd_confirmed_harder_dual_cap
...
candidate:improved_phaseddd_confirmed_harder_dual_cap      0.0714   0.0762  0.9379       -0.1175 -0.0253  0.6080        0.0617         0.0008
```

## Risk Contribution — improved_phaseddd_confirmed_harder_dual_cap

```
            sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
       taa_10m_sma             0.1120                 0.0000                 0.5554                        0.4434
dual_momentum_topn             0.0915                 0.0000                 0.4446                        0.3532
```

**Hidden concentration flagged:**
- `taa_10m_sma`: dollar weight 11.2% but risk contribution 55.5% (delta +44.3pp).
- `dual_momentum_topn`: dollar weight 9.1% but risk contribution 44.5% (delta +35.3pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +7.14% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.938 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +7.14% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.938 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +7.14% vs HRP (internal) +4.19%); on Sharpe? **YES** (cand 0.938 vs HRP (internal) 0.925).
