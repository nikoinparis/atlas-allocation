# Allocator Benchmark Audit — improved_phasemm_recovery_confirmed_sleeve_fix
...
candidate:improved_phasemm_recovery_confirmed_sleeve_fix      0.0690   0.0780  0.8853       -0.1398 -0.0262  0.4939        0.0564         0.0007
```

## Risk Contribution — improved_phasemm_recovery_confirmed_sleeve_fix

```
                      sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
composite_regime_conditioned             0.2554                 0.0000                 0.4097                        0.1543
                 taa_10m_sma             0.1350                 0.0000                 0.3216                        0.1866
          dual_momentum_topn             0.1075                 0.0000                 0.2688                        0.1613
```

**Hidden concentration flagged:**
- `composite_regime_conditioned`: dollar weight 25.5% but risk contribution 41.0% (delta +15.4pp).
- `taa_10m_sma`: dollar weight 13.5% but risk contribution 32.2% (delta +18.7pp).
- `dual_momentum_topn`: dollar weight 10.7% but risk contribution 26.9% (delta +16.1pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +6.90% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.885 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +6.90% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.885 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +6.90% vs HRP (internal) +4.19%); on Sharpe? **NO** (cand 0.885 vs HRP (internal) 0.925).
