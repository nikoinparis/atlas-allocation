# Allocator Benchmark Audit — improved_phasesss3_calm_old_low_stress_derisk
...
candidate:improved_phasesss3_calm_old_low_stress_derisk      0.0714   0.0761  0.9386       -0.1160 -0.0253  0.6155        0.0618         0.0008
```

## Risk Contribution — improved_phasesss3_calm_old_low_stress_derisk

```
            sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
       taa_10m_sma             0.1127                 0.0000                 0.5613                        0.4487
dual_momentum_topn             0.0901                 0.0000                 0.4387                        0.3486
```

**Hidden concentration flagged:**
- `taa_10m_sma`: dollar weight 11.3% but risk contribution 56.1% (delta +44.9pp).
- `dual_momentum_topn`: dollar weight 9.0% but risk contribution 43.9% (delta +34.9pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +7.14% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.939 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +7.14% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.939 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +7.14% vs HRP (internal) +4.19%); on Sharpe? **YES** (cand 0.939 vs HRP (internal) 0.925).
