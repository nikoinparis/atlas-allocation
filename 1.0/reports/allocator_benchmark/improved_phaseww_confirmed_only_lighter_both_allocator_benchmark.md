# Allocator Benchmark Audit — improved_phaseww_confirmed_only_lighter_both
...
candidate:improved_phaseww_confirmed_only_lighter_both      0.0699   0.0786  0.8893       -0.1408 -0.0264  0.4963        0.0578         0.0008
```

## Risk Contribution — improved_phaseww_confirmed_only_lighter_both

```
                      sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
composite_regime_conditioned             0.2394                 0.0000                 0.3833                        0.1439
                 taa_10m_sma             0.1396                 0.0000                 0.3366                        0.1971
          dual_momentum_topn             0.1111                 0.0000                 0.2801                        0.1690
```

**Hidden concentration flagged:**
- `composite_regime_conditioned`: dollar weight 23.9% but risk contribution 38.3% (delta +14.4pp).
...
- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +6.99% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.889 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +6.99% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.889 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +6.99% vs HRP (internal) +4.19%); on Sharpe? **NO** (cand 0.889 vs HRP (internal) 0.925).
