# Allocator Benchmark Audit — improved_phasezz_recovery_neutral_offense_rebudget
...
candidate:improved_phasezz_recovery_neutral_offense_rebudget      0.0708   0.0757  0.9347       -0.1175 -0.0251  0.6026        0.0602         0.0008
```

## Risk Contribution — improved_phasezz_recovery_neutral_offense_rebudget

```
            sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
       taa_10m_sma             0.1133                 0.0000                 0.5399                        0.4267
dual_momentum_topn             0.0976                 0.0000                 0.4601                        0.3625
```

**Hidden concentration flagged:**
- `taa_10m_sma`: dollar weight 11.3% but risk contribution 54.0% (delta +42.7pp).
- `dual_momentum_topn`: dollar weight 9.8% but risk contribution 46.0% (delta +36.2pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +7.08% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.935 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +7.08% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.935 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +7.08% vs HRP (internal) +4.19%); on Sharpe? **YES** (cand 0.935 vs HRP (internal) 0.925).
- Does the candidate beat **production** on annualised return? **YES** (cand +7.08% vs prod +6.89%); on Sharpe? **YES**.
