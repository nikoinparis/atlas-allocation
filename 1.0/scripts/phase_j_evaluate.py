...
PHASE_J_CANDIDATES = [
    "improved_phasej_robust_objective_allocator",
    "improved_phasej_margin_confidence_allocator",
    "improved_phasej_turnover_tail_allocator",
    "improved_phasej_structural_combo_allocator",
]
...
FIXED_COMPARATOR_SET = [
    ...
    OLDER_BOUNDED_REFERENCE,
    REFERENCE_PANEL_VERSION,
]
ALL_VERSIONS = FIXED_COMPARATOR_SET + PHASE_J_CANDIDATES
...
def main() -> None:
    ...
    pairwise_rows = []
    for name in ALL_VERSIONS:
        cand_full = full_df.set_index("version_name").loc[name]
        cand_holdout = holdout_df.set_index("version_name").loc[name]
    ...
    full_df.to_csv(LAYER3_DIR / "phase_j_candidate_metrics_full.csv", index=False)
    dev_df.to_csv(LAYER3_DIR / "phase_j_candidate_metrics_dev.csv", index=False)
    holdout_df.to_csv(LAYER3_DIR / "phase_j_candidate_metrics_holdout.csv", index=False)
    rolling_df.to_csv(LAYER3_DIR / "phase_j_rolling_origin_summary.csv", index=False)
    pairwise_df.to_csv(LAYER3_DIR / "phase_j_pairwise_validation.csv", index=False)
    ...
    (LAYER3_DIR / "phase_j_validation_protocol.json").write_text(json.dumps(protocol, indent=2))

    print("Saved Phase J evaluation artifacts:")
    for name in [
        "data/05_layer3_portfolio_construction/phase_j_candidate_metrics_full.csv",
        "data/05_layer3_portfolio_construction/phase_j_candidate_metrics_dev.csv",
        "data/05_layer3_portfolio_construction/phase_j_candidate_metrics_holdout.csv",
        "data/05_layer3_portfolio_construction/phase_j_rolling_origin_summary.csv",
        "data/05_layer3_portfolio_construction/phase_j_pairwise_validation.csv",
        "data/05_layer3_portfolio_construction/phase_j_validation_protocol.json",
    ]:
        print(" -", name)
