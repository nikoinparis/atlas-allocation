...
PHASE_L_CANDIDATES = [
    "improved_phasel_decision_utility_allocator",
    "improved_phasel_selective_concentration_allocator",
    "improved_phasel_tail_turnover_learning_allocator",
]
...
ALL_VERSIONS = FIXED_COMPARATOR_SET + PHASE_L_CANDIDATES
...
def main() -> None:
    ...
    tail_ref_full = full_idx.loc[TAIL_AWARE_BRANCH]
    ...
    tail_ref_holdout_returns = returns_map[TAIL_AWARE_BRANCH].tail(HOLDOUT_WEEKS)
    ...
    for name in ALL_VERSIONS:
        ...
        row = {
            ...
            "full_raw_delta_vs_tail_aware_branch": float(cand_full["raw_target_composite"] - tail_ref_full["raw_target_composite"]),
            ...
            "bootstrap_prob_vs_tail_aware_branch": moving_block_bootstrap_prob(cand_holdout_returns - tail_ref_holdout_returns),
    ...
    full_df.to_csv(LAYER3_DIR / "phase_l_candidate_metrics_full.csv", index=False)
    dev_df.to_csv(LAYER3_DIR / "phase_l_candidate_metrics_dev.csv", index=False)
    holdout_df.to_csv(LAYER3_DIR / "phase_l_candidate_metrics_holdout.csv", index=False)
    rolling_df.to_csv(LAYER3_DIR / "phase_l_rolling_origin_summary.csv", index=False)
    pairwise_df.to_csv(LAYER3_DIR / "phase_l_pairwise_validation.csv", index=False)

    protocol = {
        "phase": "Phase L",
        "purpose": "Decision-aware / robustness-aware learning allocator validation",
    ...
    (LAYER3_DIR / "phase_l_validation_protocol.json").write_text(json.dumps(protocol, indent=2))

    print("Saved Phase L evaluation artifacts:")
    for name in [
        "data/05_layer3_portfolio_construction/phase_l_candidate_metrics_full.csv",
        "data/05_layer3_portfolio_construction/phase_l_candidate_metrics_dev.csv",
        "data/05_layer3_portfolio_construction/phase_l_candidate_metrics_holdout.csv",
        "data/05_layer3_portfolio_construction/phase_l_rolling_origin_summary.csv",
        "data/05_layer3_portfolio_construction/phase_l_pairwise_validation.csv",
        "data/05_layer3_portfolio_construction/phase_l_validation_protocol.json",
    ]:
        print(" -", name)
