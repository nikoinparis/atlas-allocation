...
ALL_VERSIONS = FIXED_COMPARATOR_SET + PHASE_N_CANDIDATES
...
def main() -> None:
    ...
    phaseh_full = full_idx.loc[CURRENT_REFINED_REFERENCE]
    phasel_full = full_idx.loc[CURRENT_LEARNING_BRANCH]
    phasek_full = full_idx.loc[TAIL_AWARE_BRANCH]
    blend_full = full_idx.loc[ACTIVE_PANEL_BASELINE]
    phaseh_holdout = holdout_idx.loc[CURRENT_REFINED_REFERENCE]
    phasel_holdout = holdout_idx.loc[CURRENT_LEARNING_BRANCH]
    phasek_holdout = holdout_idx.loc[TAIL_AWARE_BRANCH]
    ...
    phaseh_holdout_returns = returns_map[CURRENT_REFINED_REFERENCE].tail(HOLDOUT_WEEKS)
    phasel_holdout_returns = returns_map[CURRENT_LEARNING_BRANCH].tail(HOLDOUT_WEEKS)
    phasek_holdout_returns = returns_map[TAIL_AWARE_BRANCH].tail(HOLDOUT_WEEKS)
    ...
    for name in ALL_VERSIONS:
        ...
        row = {
            ...
            "full_raw_delta_vs_phasel_branch": float(cand_full["raw_target_composite"] - phasel_full["raw_target_composite"]),
            "holdout_raw_delta_vs_phasel_branch": float(cand_holdout["raw_target_composite"] - phasel_holdout["raw_target_composite"]),
            "holdout_sharpe_delta_vs_phasel_branch": float(cand_holdout["sharpe"] - phasel_holdout["sharpe"]),
            "bootstrap_prob_vs_phasel_branch": safe_bootstrap(cand_holdout_returns, phasel_holdout_returns),
            "full_raw_delta_vs_phasek_branch": float(cand_full["raw_target_composite"] - phasek_full["raw_target_composite"]),
            "holdout_raw_delta_vs_phasek_branch": float(cand_holdout["raw_target_composite"] - phasek_holdout["raw_target_composite"]),
            "holdout_sharpe_delta_vs_phasek_branch": float(cand_holdout["sharpe"] - phasek_holdout["sharpe"]),
            "bootstrap_prob_vs_phasek_branch": safe_bootstrap(cand_holdout_returns, phasek_holdout_returns),
            ...
            "max_drawdown_delta_vs_phaseh_reference": float(cand_full["max_drawdown"] - phaseh_full["max_drawdown"]),
            "cvar_delta_vs_phaseh_reference": float(cand_full["cvar_5"] - phaseh_full["cvar_5"]),
            **roll.to_dict(),
        }
        row["production_rule_checks_passed"] = int(
            row["full_raw_delta_vs_production"] >= prod_rule["full_raw_composite_delta_vs_production_min"]
        ) + int(
            row["holdout_raw_delta_vs_production"] >= prod_rule["holdout_raw_composite_delta_vs_production_min"]
        ) + int(
            row["holdout_sharpe_delta_vs_production"] >= prod_rule["holdout_sharpe_delta_vs_production_min"]
        ) + int(
            row["rolling_raw_win_rate_vs_production"] >= prod_rule["rolling_raw_win_rate_vs_production_min"]
        ) + int(
            row["rolling_mean_raw_delta_vs_production"] > prod_rule["rolling_mean_raw_delta_vs_production_min"]
        ) + int(
            row["bootstrap_prob_vs_production"] >= prod_rule["holdout_bootstrap_prob_excess_return_min"]
        ) + int(
            row["max_drawdown_delta_vs_production"] >= prod_rule["max_drawdown_worsening_cap"]
        ) + int(
            row["cvar_delta_vs_production"] >= prod_rule["cvar_worsening_cap"]
        )
        row["production_rule_pass"] = bool(
            row["production_rule_checks_passed"] == 8
        )
    ...
    classification_rows = []
    for name in PHASE_N_CANDIDATES:
        row = pairwise_df.set_index("version_name").loc[name]
        if bool(row["production_rule_pass"]):
            classification = "Promote now"
        elif bool(row["shadow_rule_pass"]) or (
            row["full_raw_delta_vs_phaseh_reference"] > 0
            and row["holdout_sharpe_delta_vs_phaseh_reference"] >= 0
            and row["max_drawdown_delta_vs_phaseh_reference"] >= -0.005
            and row["cvar_delta_vs_phaseh_reference"] >= -0.001
        ):
            classification = "Conditional"
        elif row["full_raw_delta_vs_phaseh_reference"] > 0 or row["bootstrap_prob_vs_phaseh_reference"] >= 0.50:
        ...
        classification_rows.append(
            {
                ...
                "best_non_production_candidate": bool(name == best_non_production_candidate),
                "production_rule_checks_passed": int(row["production_rule_checks_passed"]),
            }
        )
    classification_df = pd.DataFrame(classification_rows)

    full_df.to_csv(LAYER3_DIR / "phase_n_candidate_metrics_full.csv", index=False)
    dev_df.to_csv(LAYER3_DIR / "phase_n_candidate_metrics_dev.csv", index=False)
    holdout_df.to_csv(LAYER3_DIR / "phase_n_candidate_metrics_holdout.csv", index=False)
    rolling_df.to_csv(LAYER3_DIR / "phase_n_rolling_origin_summary.csv", index=False)
    pairwise_df.to_csv(LAYER3_DIR / "phase_n_pairwise_validation.csv", index=False)
    classification_df.to_csv(LAYER3_DIR / "phase_n_candidate_classification.csv", index=False)

    protocol = {
        "phase": "Phase N",
        "purpose": "Ambitious ML phase evaluation",
    ...
    (LAYER3_DIR / "phase_n_validation_protocol.json").write_text(json.dumps(protocol, indent=2))

    print("Saved Phase N evaluation artifacts:")
    for name in [
        ...
        "data/05_layer3_portfolio_construction/phase_n_validation_protocol.json",
    ]:
        print(" -", name)
