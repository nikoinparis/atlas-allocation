"""Phase OO — composite_regime_conditioned sleeve-internal cash audit.

Focus:
1. Diagnose whether composite_regime_conditioned's internal BIL architecture
   is appropriate stress defense or excessive favorable-state drag.
2. Attribute hidden BIL drag to the sleeve's own internal cash tiers.
3. Test up to three narrow in-pipeline fixes using the production builder.
"""
...
PHASE_OO_CANDIDATES = [
    "improved_phaseoo_composite_recovery_cash_relief",
    "improved_phaseoo_composite_neutral_cash_relief",
    "improved_phaseoo_composite_combined_cash_relief",
]
AUDIT_DIR = ROOT / "data" / "research" / "phase_oo_composite_regime_cash"
REPORT_PATH = ROOT / "docs" / "research" / "2026-04-27_phase_oo_composite_regime_cash_report.md"
...
def build_candidates() -> None:
    env = os.environ.copy()
    env["BUILD_VERSION_NAMES"] = ",".join([PRODUCTION, SHADOW] + PHASE_OO_CANDIDATES)
...
def candidate_position_maps(base_positions: dict[str, pd.DataFrame], state_history: pd.DataFrame) -> dict[str, dict[str, pd.DataFrame]]:
    return {
        ...
        "improved_phaseoo_composite_recovery_cash_relief": apply_internal_redeploy(
            base_positions,
            state_history,
            target_sleeves=[TARGET_SLEEVE],
            redeploy_config={"recovery_fragile": 0.24, "recovery_confirmed": 0.18},
            strong_neutral_fraction=0.0,
        ),
        "improved_phaseoo_composite_neutral_cash_relief": apply_internal_redeploy(
            base_positions,
            state_history,
            target_sleeves=[TARGET_SLEEVE],
            redeploy_config={},
        ...
        "improved_phaseoo_composite_combined_cash_relief": apply_internal_redeploy(
            base_positions,
            state_history,
            target_sleeves=[TARGET_SLEEVE],
...
def offensive_weight(row: pd.Series) -> float:
    cols = [c for c in row.index if c not in DEFENSIVE_ETFS.union({"BIL"})]
    return float(row.reindex(cols).fillna(0.0).sum())


def defensive_weight(row: pd.Series) -> float:
    cols = [c for c in row.index if c in DEFENSIVE_ETFS and c != "BIL"]
    return float(row.reindex(cols).fillna(0.0).sum())


def composite_exposure_by_state(positions: pd.DataFrame, state_history: pd.DataFrame) -> pd.DataFrame:
    merged = positions.join(state_history[["state_bucket"]], how="inner")
    rows = []
    asset_cols = [c for c in positions.columns]
    offense_cols = [c for c in asset_cols if c not in DEFENSIVE_ETFS.union({"BIL"})]
    defense_cols = [c for c in asset_cols if c not in offense_cols and c != "BIL"]
    for state, sub in merged.groupby("state_bucket", observed=False):
        row = {
            "state": state,
            "n_weeks": int(len(sub)),
            "avg_BIL": float(sub.get("BIL", pd.Series(0.0, index=sub.index)).mean()),
            "avg_SPY": float(sub.get("SPY", pd.Series(0.0, index=sub.index)).mean()),
            "avg_offense": float(sub.reindex(columns=offense_cols, fill_value=0.0).sum(axis=1).mean()),
            "avg_defense": float(sub.reindex(columns=defense_cols, fill_value=0.0).sum(axis=1).mean()),
            "avg_total_risky": 1.0 - float(sub.get("BIL", pd.Series(0.0, index=sub.index)).mean()),
            "avg_HYG": float(sub.get("HYG", pd.Series(0.0, index=sub.index)).mean()),
            "avg_LQD": float(sub.get("LQD", pd.Series(0.0, index=sub.index)).mean()),
            "avg_TLT": float(sub.get("TLT", pd.Series(0.0, index=sub.index)).mean()),
            "avg_GLD": float(sub.get("GLD", pd.Series(0.0, index=sub.index)).mean()),
            "avg_VNQ": float(sub.get("VNQ", pd.Series(0.0, index=sub.index)).mean()),
            "avg_EFA": float(sub.get("EFA", pd.Series(0.0, index=sub.index)).mean()),
            "avg_QQQ": float(sub.get("QQQ", pd.Series(0.0, index=sub.index)).mean()),
        }
        rows.append(row)
...
def composite_bil_source_diagnostics(positions: pd.DataFrame, state_history: pd.DataFrame, prod_sleeve_weights: pd.DataFrame) -> pd.DataFrame:
    merged = positions.join(state_history[["state_bucket"]], how="inner")
    prod_weight = prod_sleeve_weights.get(TARGET_SLEEVE, pd.Series(0.0, index=prod_sleeve_weights.index)).reindex(merged.index).fillna(0.0)
    merged["prod_sleeve_weight"] = prod_weight
    merged["prod_hidden_bil_contrib"] = merged.get("BIL", pd.Series(0.0, index=merged.index)) * merged["prod_sleeve_weight"]
    rows = []
    for state, sub in merged.groupby("state_bucket", observed=False):
        bil = sub["BIL"]
        tier_counts = bil.round(4).value_counts(normalize=True)
        dominant_tier = float(tier_counts.idxmax()) if not tier_counts.empty else np.nan
        rows.append(
            {
                "state": state,
                "n_weeks": int(len(sub)),
                ...
                "pct_BIL_25": float((bil == 0.25).mean()),
                "pct_BIL_4375": float((bil == 0.4375).mean()),
                "pct_BIL_65": float((bil == 0.65).mean()),
                "pct_BIL_7375": float((bil == 0.7375).mean()),
                "pct_BIL_100": float((bil == 1.0).mean()),
                "dominant_BIL_tier": dominant_tier,
                "avg_prod_hidden_bil_contrib": float(sub["prod_hidden_bil_contrib"].mean()),
                "source_classification": (
                    "defensive_fallback_dominant" if float(bil.mean()) >= 0.45 else
                    "tiered_fallback_drag" if float((bil == 0.25).mean()) >= 0.40 else
                    "mixed"
                ),
            }
        )
...
def component_breakdown() -> pd.DataFrame:
    sig = pd.read_csv(ROOT / "data" / "03_layer2a_strategy_logic" / "strategy_signal_weight_history.csv", parse_dates=["Date"])
    sub = sig[sig["strategy_name"] == TARGET_SLEEVE].copy()
    rows = []
    for signal_name, grp in sub.groupby("signal_name", observed=False):
        rows.append(
            {
                "signal_name": signal_name,
                "mean_weight": float(grp["weight"].mean()),
                "median_weight": float(grp["weight"].median()),
                "min_weight": float(grp["weight"].min()),
                "max_weight": float(grp["weight"].max()),
                "n_dates": int(grp["Date"].nunique()),
            }
        )
    out = pd.DataFrame(rows).sort_values("mean_weight", ascending=False).reset_index(drop=True)
    out["component_role_guess"] = np.where(
        out["signal_name"].isin(["multi_mom_invvol", "xsmom_global", "residual_momentum"]),
        "trend_momentum_core",
        np.where(
            out["signal_name"].isin(["quality_proxy", "value_proxy", "bab_proxy", "carry_proxy"]),
            "cross_sectional_balancer",
            "short_term_overlay",
        ),
    )
    return out
...
def candidate_diagnostics(
    ...
    prod_hidden_contrib = prod_positions.get("BIL", pd.Series(0.0, index=prod_positions.index)).fillna(0.0) * prod_target_weight
    base = pd.DataFrame(
        {
            "state": state_history["state_bucket"],
            "prod_internal_bil": prod_positions.get("BIL", pd.Series(0.0, index=prod_positions.index)).fillna(0.0),
            "prod_hidden_bil_contrib": prod_hidden_contrib,
        },
        index=state_history.index,
    )
    rows = []
    defense_cols = [c for c in prod_positions.columns if c in DEFENSIVE_ETFS and c != "BIL"]
    for name in candidate_names:
        sw = roc.load_portfolio_sleeve_weights(name)
        pw = roc.load_portfolio_weights(name)
        ret = roc.load_portfolio_returns(name)
        if sw is None or pw is None or ret is None:
            continue
        ...
        df = base.copy()
        df["internal_bil"] = pos.get("BIL", pd.Series(0.0, index=pos.index)).fillna(0.0)
        df["hidden_bil_contrib"] = df["internal_bil"] * target_weight
        df["sleeve_weight"] = target_weight
        df["internal_spy"] = pos.get("SPY", pd.Series(0.0, index=pos.index)).fillna(0.0)
        df["internal_offense"] = pos.drop(columns=[c for c in pos.columns if c in DEFENSIVE_ETFS.union({"BIL"})], errors="ignore").sum(axis=1)
        df["internal_defense"] = pos.reindex(columns=defense_cols, fill_value=0.0).sum(axis=1)
        state_rows = state_summary_row(name, roc.load_weekly_returns()["SPY"], state_history)
        state_lookup = state_rows.set_index("state") if not state_rows.empty else pd.DataFrame()
        for state, sub in df.groupby("state", observed=False):
            rows.append(
                {
                    "name": name,
                    "state": state,
                    "avg_composite_internal_bil": float(sub["internal_bil"].mean()),
                    "composite_bil_reduction_vs_prod": float(sub["prod_internal_bil"].mean() - sub["internal_bil"].mean()),
                    "avg_composite_hidden_bil_contrib": float(sub["hidden_bil_contrib"].mean()),
                    "composite_hidden_bil_reduction_vs_prod": float(sub["prod_hidden_bil_contrib"].mean() - sub["hidden_bil_contrib"].mean()),
                    "avg_composite_sleeve_weight": float(sub["sleeve_weight"].mean()),
                    "avg_internal_spy": float(sub["internal_spy"].mean()),
                    "avg_internal_offense": float(sub["internal_offense"].mean()),
                    "avg_internal_defense": float(sub["internal_defense"].mean()),
                    "avg_final_portfolio_bil": float(pw.get("BIL", pd.Series(0.0, index=pw.index)).reindex(sub.index).fillna(0.0).mean()),
                    "avg_final_portfolio_spy": float(pw.get("SPY", pd.Series(0.0, index=pw.index)).reindex(sub.index).fillna(0.0).mean()),
                    "state_ann_return": float(state_lookup.loc[state, "ann_return"]) if state in state_lookup.index else np.nan,
                    "state_sharpe": float(state_lookup.loc[state, "sharpe"]) if state in state_lookup.index else np.nan,
                    "state_mean_weekly_return": float(state_lookup.loc[state, "mean_weekly_return"]) if state in state_lookup.index else np.nan,
                }
            )
    out = pd.DataFrame(rows)
    if not out.empty:
...
def build_selection_table(metrics_df: pd.DataFrame, candidate_diag: pd.DataFrame, state_summary: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    ...
    rows = []
    for candidate in PHASE_OO_CANDIDATES:
        ...
        avg_bil_reduction = float(diag_sub["composite_bil_reduction_vs_prod"].mean()) if not diag_sub.empty else np.nan
        ...
        cond_bil = np.isfinite(avg_bil_reduction) and avg_bil_reduction > 0 and np.isfinite(avg_hidden_reduction) and avg_hidden_reduction > 0
        cond_translation = cond_bil and (ann_delta_pp > 0 or sharpe_delta > 0)
        ...
        fail_reasons = "; ".join(
            reason for reason in [
                ...
                f"cash reduction not translating ({avg_hidden_reduction:+.4f})" if not cond_translation else "",
            ] if reason
        ) or "none"
        rows.append(
            {
                ...
                "avg_composite_bil_reduction_vs_prod": avg_bil_reduction,
    ...
    best = selection.sort_values(
        by=["passes_all_gates", "sharpe_delta_vs_prod", "ann_return_delta_pp_vs_prod", "avg_composite_hidden_bil_reduction_vs_prod"],
...
def genuine_improvement(selection: pd.DataFrame, candidate: str) -> bool:
    ...
    return bool(
        ...
        and r["avg_composite_hidden_bil_reduction_vs_prod"] > 0
    )


def write_report(
    exposure_by_state: pd.DataFrame,
    bil_source_df: pd.DataFrame,
    ...
    biggest_source = "No clear internal source isolated."
    if not bil_source_df.empty:
        source_row = bil_source_df.sort_values("avg_prod_hidden_bil_contrib", ascending=False).iloc[0]
        biggest_source = (
            f"{source_row['state']}: dominant tier {source_row['dominant_BIL_tier']:.4f}, "
            f"avg hidden contribution {source_row['avg_prod_hidden_bil_contrib']:.4f}, "
            f"classification {source_row['source_classification']}."
        )
    lines = [
        ...
        *[f"{cmd}\n" for cmd in COMMAND_LOG],
        ...
        exposure_by_state.to_string(index=False, float_format=lambda x: f"{x:.4f}") if not exposure_by_state.empty else "No exposure table available.",
        "\n```\n\n",
        "## Biggest Internal Source Of BIL / Cash Behavior\n",
        f"- {biggest_source}\n\n",
        "## BIL Source Diagnostics\n```\n",
        bil_source_df.to_string(index=False, float_format=lambda x: f"{x:.4f}") if not bil_source_df.empty else "No BIL-source diagnostics available.",
        "\n```\n\n",
        "## Component Breakdown\n```\n",
        component_df.to_string(index=False, float_format=lambda x: f"{x:.4f}") if not component_df.empty else "No component breakdown available.",
        "\n```\n\n",
        ...
        f"- This sleeve-internal cash architecture path should {'continue' if final_decision in {'KEEP AS SHADOW', 'KEEP AS PRODUCTION'} else 'not continue in its current narrow form'}.\n",
        f"- Recommended next phase if this fails: {'broader composite_regime_conditioned sleeve redesign or defensive sleeve architecture rethink' if final_decision in {'REJECT', 'NEEDS FIX BEFORE JUDGMENT', 'DIAGNOSTIC ONLY / NO SAFE CANDIDATE'} else 'follow-on robustness validation on the best composite-only relief'}.\n",
    ]
...
def main() -> None:
    ...
    composite_positions = base_positions[TARGET_SLEEVE].reindex(state_history.index)
    exposure_by_state = composite_exposure_by_state(composite_positions, state_history)
    exposure_by_state.to_csv(AUDIT_DIR / "phase_oo_composite_regime_exposure_by_state.csv", index=False)

    bil_source_df = composite_bil_source_diagnostics(composite_positions, state_history, prod_sleeve_weights)
    bil_source_df.to_csv(AUDIT_DIR / "phase_oo_composite_regime_bil_source_diagnostics.csv", index=False)

    component_df = component_breakdown()
    component_df.to_csv(AUDIT_DIR / "phase_oo_composite_regime_component_breakdown.csv", index=False)

    build_candidates()

    benchmark_returns = roc.load_weekly_returns()["SPY"]
    metrics_rows = [metric_row(name, benchmark_returns, state_history) for name in [PRODUCTION, SHADOW] + PHASE_OO_CANDIDATES]
    metrics_df = pd.DataFrame(metrics_rows)
    metrics_df.to_csv(OUTPUT_DIR / "phase_oo_candidate_metrics_full.csv", index=False)

    state_summary_parts = [state_summary_row(name, benchmark_returns, state_history) for name in [PRODUCTION, SHADOW] + PHASE_OO_CANDIDATES]
    state_summary = pd.concat(state_summary_parts, ignore_index=True) if any(not df.empty for df in state_summary_parts) else pd.DataFrame()
    ...
    state_summary.to_csv(OUTPUT_DIR / "phase_oo_state_summary.csv", index=False)

    candidate_diag = candidate_diagnostics(PHASE_OO_CANDIDATES, prod_sleeve_weights, state_history, position_maps)
    candidate_diag.to_csv(AUDIT_DIR / "phase_oo_candidate_diagnostics.csv", index=False)

    selection, best_candidate = build_selection_table(metrics_df, candidate_diag, state_summary)
    selection.to_csv(OUTPUT_DIR / "phase_oo_selection_table.csv", index=False)

    quick_verdict = "DIAGNOSTIC ONLY / NO SAFE CANDIDATE"
    committee_report = roc.REPORTS_DIR / "research_committee" / "phase_oo_no_candidate.md"
    ran_layer56 = False
    if best_candidate:
    ...
    protocol = {
        ...
        "candidate_names": PHASE_OO_CANDIDATES,
        "commands_executed": ["python scripts/phase_oo_composite_regime_cash_audit.py", *COMMAND_LOG],
        "diagnostic_outputs": [
            "data/research/phase_oo_composite_regime_cash/phase_oo_composite_regime_exposure_by_state.csv",
            "data/research/phase_oo_composite_regime_cash/phase_oo_composite_regime_bil_source_diagnostics.csv",
            "data/research/phase_oo_composite_regime_cash/phase_oo_composite_regime_component_breakdown.csv",
            "data/research/phase_oo_composite_regime_cash/phase_oo_candidate_diagnostics.csv",
        ],
        "candidate_outputs": [
            "data/05_layer3_portfolio_construction/phase_oo_candidate_metrics_full.csv",
            "data/05_layer3_portfolio_construction/phase_oo_state_summary.csv",
            "data/05_layer3_portfolio_construction/phase_oo_selection_table.csv",
        ],
        "best_candidate": best_candidate,
        "quick_verdict": quick_verdict,
        "layer56_quick_audits_ran": ran_layer56,
    }
    (OUTPUT_DIR / "phase_oo_protocol.json").write_text(json.dumps(protocol, indent=2))
