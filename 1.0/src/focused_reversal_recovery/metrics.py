"""Metrics for focused reversal recovery research."""
...
def subperiod_id(dates: pd.Series, n_parts: int = 3) -> pd.Series:
    ordered = pd.Series(pd.to_datetime(dates)).rank(method="dense")
    if ordered.max() <= 1:
        return pd.Series(1, index=dates.index)
    bucket = np.ceil(ordered / ordered.max() * n_parts)
    return bucket.clip(1, n_parts).astype(int)
