#!/usr/bin/env python3
"""Acquire a frozen Yahoo research cache for the launch-to-current IDX80 union."""
...
MEMBERSHIP_ROOT = ROOT / "data" / "indonesia_idx80_extended_history_vintages"
OUTPUT_ROOT = ROOT / "data" / "indonesia_idx80_extended_price_vintages"
...
def main() -> int:
    ...
    if not membership_manifest["claims"]["complete_from_idx80_launch"]:
        raise ValueError("membership vintage is not complete from IDX80 launch")
    ...
    vintage_id = f"{stamp}-idx80-extended-prices-v1"
    destination = OUTPUT_ROOT / vintage_id
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="idx80-extended-prices-", dir=ROOT / "data") as temporary:
        export = Path(temporary) / "export"
        export.mkdir()
        completed = subprocess.run(
            [
                "podman", "run", "--rm", "-v", f"{export.resolve()}:/export:Z", IMAGE,
                "--symbols", ",".join(symbols), "--period", "10y", "--output", "/export"
            ],
            text=True,
            capture_output=True,
        )
        ...
        if completed.returncode != 0 or metadata.get("status") != "complete":
            raise RuntimeError(
                f"extended IDX80 price acquisition failed: {completed.stdout} {completed.stderr} "
    ...
    files = {
        ...
        for path in sorted(destination.iterdir()) if path.is_file() and path.name != "manifest.json"
    }
    manifest = {
        "vintage_id": vintage_id,
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "purpose": "research-only prices for launch-to-current IDX80 membership union",
        ...
        "claims": {
            "membership_complete_from_idx80_launch": True,
            "membership_official_direct_complete": False,
            "price_vendor_revision_frozen": True,
            "licensed_for_redistribution": False,
            "delisting_complete": False,
            "total_return_benchmarks": False,
