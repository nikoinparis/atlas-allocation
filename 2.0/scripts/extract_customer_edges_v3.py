...
# v2 scored 73.3%, and 11 of its 16 errors were the filer's OWN segment names:
# "Consumer Electronics sales represented 70.3% of our net sales" parses exactly like a
# customer sentence. The signal that separates them is corporate form. Real customers
# carry a hard suffix (Cisco Systems, Inc.; McKesson Corporation) or are known megacaps;
# segment labels never do (Platform Solutions, Metrology Solutions, Data Communications).
HARD_SUFFIX = (r"Inc|Corp|Corporation|Company|Co|LLC|L\.L\.C|Ltd|Limited|LP|L\.P|plc|PLC|"
               r"N\.V|S\.A|AG|SE|AB|GmbH|Oyj|ASA|Bhd")
SOFT_SUFFIX = (r"Group|Holdings|Technologies|Technology|Systems|Solutions|Industries|"
               r"Enterprises|Stores|Motors|Airlines|Pharmaceuticals|Networks|Labs|"
               r"Laboratories|Communications|Electronics|Semiconductor|Bank|Energy|Partners")
SUFFIX = HARD_SUFFIX + r"|" + SOFT_SUFFIX
...
HARD_ENTITY = re.compile(r"(?:" + HARD_SUFFIX + r")\.?$", re.I)
# Immediately-preceding customer grammar, required for soft-suffix names.
LEAD_IN = re.compile(r"(?:sales?|revenues?|purchases?)\s+(?:to|from)\s+(?:\w+\s+){0,3}$|"
                     r"(?:customer|client|reseller|distributor|partner)s?[,:]?\s+"
                     r"(?:\w+\s+){0,4}$|(?:and|,)\s+$", re.I)
# A segment word immediately after marks the phrase as the filer's own business line.
SEGMENT_TAIL = re.compile(r"^\s*(?:segment|division|business|product|products|offerings?|"
                          r"sales|revenues?|unit|units|group\s+revenue)\b", re.I)
...
def extract_from_text(text: str, cik: str) -> list[dict]:
    ...
    for sentence in SENTENCE_SPLIT.split(text):
        ...
        for m in cands:
            ...
            if OWN_PRODUCT.search(before[-24:]):
                continue                              # "our <Entity> products"
            if SEGMENT_TAIL.match(after):
                continue                              # "<Entity> segment/sales/products"
            raw_name = m.group(1).strip(" .,")
            if not HARD_ENTITY.search(raw_name) and not BARE.fullmatch(raw_name):
                if not LEAD_IN.search(before):
                    continue                          # soft suffix with no customer lead-in
...
def main() -> int:
    ...
    print(f"filings yielding >=1 edge: {len(with_edge)}  ({len(with_edge)/n:.1%})   v2: 5.4%")
    print(f"edges                    : {len(edges)}   v2: 249")
    print(f"distinct named customers : {len(names)}   v2: 141")
    if edges:
    ...
    for s, c in names.most_common(20):
        print(f"   {c:>3}  {s}")
    json.dump(edges, open(outdir / "edges_v3.json", "w"), indent=1)
    print(f"\nwrote {outdir/'edges_v3.json'}")
    return 0
