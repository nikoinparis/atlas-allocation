#!/usr/bin/env python3
"""Sealed, unlevered, holdings-level cross-strategy residual allocator.
...
CONFIG = ROOT / "config/sec_cross_strategy_residual_allocator_v2.json"
SOURCE = ROOT / "dashboard/public/return-first-dashboard.json"
SECTOR_MAP = ROOT / "data/cross_strategy_concentration_map_v1/sector_map.csv"
...
def concentration(weights: dict[str, float], sector_map: dict, look_through: dict) -> dict:
    issuer = {symbol: abs(weight) for symbol, weight in weights.items()
              if sector_map.get(symbol, {}).get("instrument_type") != "exchange_traded_product"}
