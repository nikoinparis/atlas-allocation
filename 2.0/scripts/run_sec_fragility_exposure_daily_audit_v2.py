...
CONFIG = ROOT / "config/sec_fragility_exposure_daily_audit_v2.json"
...
def simulate_component_blend(components: pd.DataFrame, target_by_week: pd.DataFrame) -> pd.DataFrame:
    """Hold component notionals plus balancing cash within each week."""
    ...
    for date, returns in components.iterrows():
        ...
        if prior_label != label:
            ...
            cash = total * (1.0 - float(target.sum()))
            prior_label = label
        positions = positions * (1.0 + returns.fillna(0.0))
        total = float(positions.sum()) + cash
