...
CONFIG = ROOT / "config/unlevered_concentration_caps_v2.json"
OUTPUT = ROOT / "evidence/unlevered_concentration_caps_v2"
PUBLIC = ROOT / "dashboard/public/concentration-caps-v2.json"
CASH = "cash::USD"
...
def apply_caps(weights: dict[str, float], sector_map: dict, look_through: dict, caps: dict, solver: dict) -> dict[str, float]:
    ...
    if not solver.get("redistribute"):
        return {s: w for s, w in current.items() if w > solver["tolerance"]}

    # Redistribute the released weight into whatever still has headroom.
    released = sum(weights.values()) - sum(current.values())
    for _ in range(int(solver["max_iterations"])):
        if released <= solver["tolerance"]:
            break
        headroom: dict[str, float] = {}
        sectors_now = sector_exposure(current, sector_map, look_through)
        traded_total = sum(w for s, w in current.items() if classify(s, sector_map) == "exchange_traded_product")
        for symbol, weight in current.items():
            kind_ = classify(symbol, sector_map)
            if kind_ == "data_artifact":
                continue
            if kind_ == "exchange_traded_product":
                room = min(caps["max_single_exchange_traded_weight"] - weight,
                           caps["max_total_exchange_traded_weight"] - traded_total)
            else:
                room = caps["max_single_issuer_weight"] - weight
            # a name can only absorb what its sector can still absorb
            for sector, share in sector_shares(symbol, sector_map, look_through).items():
                if share > 0:
                    room = min(room, (caps["max_look_through_sector_weight"] - sectors_now.get(sector, 0.0)) / share)
            if room > solver["tolerance"]:
                headroom[symbol] = room
        capacity = sum(headroom.values())
        if capacity <= solver["tolerance"]:
            break
        placing = min(released, capacity)
        for symbol, room in headroom.items():
            current[symbol] += placing * (room / capacity)
        released -= placing

    return {s: w for s, w in current.items() if w > solver["tolerance"]}


def sector_shares(symbol: str, sector_map: dict, look_through: dict) -> dict[str, float]:
    kind_ = classify(symbol, sector_map)
    if kind_ == "exchange_traded_product":
        return {k: float(v) for k, v in look_through.get(symbol, {}).items()}
    if kind_ == "data_artifact":
        return {}
    return {sector_of(symbol, sector_map): 1.0}
