# Allocator Benchmark Audit — improved_phase7_stretch_target
...
production:improved_phase2b_regime_confidence_boost      0.0689   0.0779  0.8848       -0.1398 -0.0262  0.4932        0.0562         0.0007
           candidate:improved_phase7_stretch_target      0.0788   0.0851  0.9262       -0.1528 -0.0281  0.5156        0.0815         0.0011
```

## Risk Contribution — improved_phase7_stretch_target

```
            sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
dual_momentum_topn             0.0997                 0.0000                 0.6594                        0.5597
       taa_10m_sma             0.0568                 0.0000                 0.3406                        0.2838
```

**Hidden concentration flagged:**
- `dual_momentum_topn`: dollar weight 10.0% but risk contribution 65.9% (delta +56.0pp).
- `taa_10m_sma`: dollar weight 5.7% but risk contribution 34.1% (delta +28.4pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +7.88% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.926 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +7.88% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.926 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +7.88% vs HRP (internal) +4.19%); on Sharpe? **YES** (cand 0.926 vs HRP (internal) 0.925).
- Does the candidate beat **production** on annualised return? **YES** (cand +7.88% vs prod +6.89%); on Sharpe? **YES**.
