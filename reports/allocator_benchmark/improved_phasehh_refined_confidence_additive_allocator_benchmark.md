# Allocator Benchmark Audit — improved_phasehh_refined_confidence_additive
...
candidate:improved_phasehh_refined_confidence_additive      0.0690   0.0778  0.8862       -0.1374 -0.0262  0.5021        0.0562         0.0007
```

## Risk Contribution — improved_phasehh_refined_confidence_additive

```
                      sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
composite_regime_conditioned             0.2547                 0.0000                 0.4105                        0.1558
                 taa_10m_sma             0.1338                 0.0000                 0.3198                        0.1860
          dual_momentum_topn             0.1074                 0.0000                 0.2697                        0.1623
```

**Hidden concentration flagged:**
- `composite_regime_conditioned`: dollar weight 25.5% but risk contribution 41.0% (delta +15.6pp).
- `taa_10m_sma`: dollar weight 13.4% but risk contribution 32.0% (delta +18.6pp).
- `dual_momentum_topn`: dollar weight 10.7% but risk contribution 27.0% (delta +16.2pp).

## Baseline Challenge

- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +6.90% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.886 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +6.90% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.886 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +6.90% vs HRP (internal) +4.19%); on Sharpe? **NO** (cand 0.886 vs HRP (internal) 0.925).
- Does the candidate beat **production** on annualised return? **YES** (cand +6.90% vs prod +6.89%); on Sharpe? **YES**.
