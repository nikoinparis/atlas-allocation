# Allocator Benchmark Audit — improved_phaseii_good_state_participation_light
...
candidate:improved_phaseii_good_state_participation_light      0.0691   0.0781  0.8853       -0.1398 -0.0262  0.4944        0.0563         0.0007
```

## Risk Contribution — improved_phaseii_good_state_participation_light

```
                      sleeve  avg_dollar_weight  avg_risk_contribution  risk_contribution_pct  risk_minus_dollar_weight_pct
composite_regime_conditioned             0.2550                 0.0000                 0.4104                        0.1553
                 taa_10m_sma             0.1340                 0.0000                 0.3199                        0.1859
          dual_momentum_topn             0.1075                 0.0000                 0.2697                        0.1622
```

**Hidden concentration flagged:**
- `composite_regime_conditioned`: dollar weight 25.5% but risk contribution 41.0% (delta +15.5pp).
...
- Does the candidate beat **Equal Weight** on annualised return? **YES** (cand +6.91% vs Equal Weight +5.45%); on Sharpe? **YES** (cand 0.885 vs Equal Weight 0.777).
- Does the candidate beat **Inverse Vol** on annualised return? **YES** (cand +6.91% vs Inverse Vol +4.72%); on Sharpe? **YES** (cand 0.885 vs Inverse Vol 0.863).
- Does the candidate beat **HRP (internal)** on annualised return? **YES** (cand +6.91% vs HRP (internal) +4.19%); on Sharpe? **NO** (cand 0.885 vs HRP (internal) 0.925).
- Does the candidate beat **production** on annualised return? **YES** (cand +6.91% vs prod +6.89%); on Sharpe? **YES**.
