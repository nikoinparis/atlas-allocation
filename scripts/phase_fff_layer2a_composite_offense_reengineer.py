...
EEE1 (`improved_phaseeee_smoothed_near_exclude_dual`) is the strongest
architecture-reference shadow but recovery_confirmed remains -0.36pp vs
production. The allocator-side weak-sleeve pruning branch is approaching
exhaustion.

Phase FFF moves to Layer 2A: rebuild the composite_regime_offense_component
itself by filtering its source ETF list (the underlying composite_regime_conditioned
positions are split into offense/defense/cash by ETF). EEE1 architecture
preserved; only the offense_component construction changes.

Four candidates:
  FFF1 = improved_phasefff_recovery_quality_filtered_offense
         (drop PDBC, DBA, EWJ — keep SPY/QQQ/IWM/EFA/VEA/VWO/VNQ)
  FFF2 = improved_phasefff_recovery_confirmed_tilted_offense
         (core equity only: SPY/QQQ/IWM/EFA/VEA/VWO)
  FFF3 = improved_phasefff_robust_composite_offense
         (drop only commodities: PDBC, DBA)
  FFF4 = improved_phasefff_conservative_offense_polish
         (drop only PDBC — minimum-shift safety variant)
"""
...
FFF3 = "improved_phasefff_robust_composite_offense"
FFF4 = "improved_phasefff_conservative_offense_polish"
PHASE_FFF_CANDIDATES = [FFF1, FFF2, FFF3, FFF4]
REFERENCES = [PRODUCTION, SHADOW, DDD1, EEE1]
...
OUT_DATA = roc.ROOT / "data" / "research" / "phase_fff_layer2a_composite_offense_reengineer"
...
def run_pipeline(version_names: list[str]) -> None:
    ...
    print(f"[Phase FFF] invoking build_improvement_artifacts.py with {len(version_names)} versions")
...
def evaluate(summary: pd.DataFrame, all_state: dict[str, dict]) -> tuple[str, str, list[dict]]:
    cands = summary[summary["name"].isin(PHASE_FFF_CANDIDATES)].copy()
    ...
    for _, r in cands.iterrows():
        ...
        rows.append({
            ...
            "sharpe_vs_eee1": sharpe_vs_eee1, "mdd_imp_pp_vs_prod": mdd_imp_pp,
    ...
    if not shadow.empty:
        ...
        return b["name"], f"Selected {b['name']} as KEEP AS SHADOW (RC repair vs EEE1).", decision.to_dict("records")
    least = decision.sort_values(["sharpe_imp_vs_prod", "ann_imp_pp_vs_prod"], ascending=[False, False]).iloc[0]
    return "", f"NO Phase FFF candidate passes any track. Best diagnostic: {least['name']}; failure reasons: {least['fail_reasons_strict']}.", decision.to_dict("records")


def main():
    if "--no-rebuild" not in sys.argv:
        run_pipeline(PHASE_FFF_CANDIDATES + REFERENCES)
    state = load_state()

    # Headlines
    rows = []
    all_state_metrics: dict[str, dict] = {}
    for name in PHASE_FFF_CANDIDATES + REFERENCES:
        h = headline(name)
        if h: rows.append(h)
        all_state_metrics[name] = state_metrics(name, state)
    summary = pd.DataFrame(rows)
    summary.to_csv(roc.LAYER3_DIR / "phase_fff_candidate_metrics_full.csv", index=False)

    # Save component construction diagnostics
    construction = pd.DataFrame([
        {"version": EEE1, "internal_redeploy": "phaseyy_decomposed",
         "offense_component_etfs": "SPY,QQQ,IWM,EFA,VEA,VWO,EWJ,VNQ,PDBC,DBA"},
        {"version": FFF1, "internal_redeploy": "phasefff_quality_filtered",
         "offense_component_etfs": "SPY,QQQ,IWM,EFA,VEA,VWO,VNQ"},
        {"version": FFF2, "internal_redeploy": "phasefff_core_equity",
         "offense_component_etfs": "SPY,QQQ,IWM,EFA,VEA,VWO"},
        {"version": FFF3, "internal_redeploy": "phasefff_robust",
         "offense_component_etfs": "SPY,QQQ,IWM,EFA,VEA,VWO,EWJ,VNQ"},
        {"version": FFF4, "internal_redeploy": "phasefff_polish",
         "offense_component_etfs": "SPY,QQQ,IWM,EFA,VEA,VWO,EWJ,VNQ,DBA"},
    ])
    construction.to_csv(OUT_DATA / "phase_fff_component_construction_diagnostics.csv", index=False)

    # state-by-state
    prod_ret = roc.load_portfolio_returns(PRODUCTION)
    state_rows = []
    for name in PHASE_FFF_CANDIDATES:
        ret = roc.load_portfolio_returns(name)
        if ret is None: continue
        df = pd.concat([ret["net_return"].rename("fff"), prod_ret["net_return"].rename("prod")], axis=1).join(
            state[["market_state"]], how="inner"
        ).dropna()
        for s, sub in df.groupby("market_state"):
            state_rows.append({"candidate": name, "state": s, "n_weeks": int(len(sub)),
                                "fff_mean_wkly": float(sub["fff"].mean()),
                                "prod_mean_wkly": float(sub["prod"].mean()),
                                "delta_mean_wkly": float(sub["fff"].mean() - sub["prod"].mean()),
                                "fff_minus_prod_cumulative": float(((1+sub["fff"]).prod()-1) - ((1+sub["prod"]).prod()-1))})
    pd.DataFrame(state_rows).to_csv(roc.LAYER3_DIR / "phase_fff_state_summary.csv", index=False)

    best, rationale, recs = evaluate(summary, all_state_metrics)
    pd.DataFrame(recs).to_csv(OUT_DATA / "phase_fff_candidate_diagnostics.csv", index=False)
    pd.DataFrame([{"best_candidate": best, "rationale": rationale}] + recs).to_csv(
        roc.LAYER3_DIR / "phase_fff_selection_table.csv", index=False)

    print("\n=== Phase FFF candidate summary ===")
    ...
    print("\n=== Phase FFF selection records ===")
    print(pd.DataFrame(recs)[["name","ann_imp_pp_vs_prod","sharpe_imp_vs_prod","sharpe_vs_eee1","turnover_ratio_vs_prod","recovery_confirmed_ann_delta_pp_vs_prod","recovery_confirmed_ann_delta_pp_vs_eee1","recovery_fragile_ann_delta_pp_vs_eee1","stressed_panic_ann_delta_pp","passes_strict_gates","passes_challenger_track","passes_shadow_track","fail_reasons_strict"]].to_string(index=False, float_format=lambda x: f"{x:.4f}"))
    print(f"\n{rationale}")

    protocol = {
        "phase": "Phase FFF — Layer 2A composite_regime_offense_component re-engineering",
        "candidates": PHASE_FFF_CANDIDATES,
        "production_pin": PRODUCTION, "shadow_pin": SHADOW,
        "eee1_reference": EEE1, "ddd1_reference": DDD1,
        "best_candidate": best, "rationale": rationale,
    }
    (roc.LAYER3_DIR / "phase_fff_protocol.json").write_text(json.dumps(protocol, indent=2))
    print("\nSaved Phase FFF artifacts.")
    return best
