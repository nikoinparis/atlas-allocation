...
PRIOR_BOUNDED_REFERENCE = "improved_phasec_state_conditioned_map"
...
PHASE_H_CANDIDATES = [
    ...
    "improved_phaseh_refined_learned_allocator",
    "improved_phaseh_refined_concentration_allocator",
    "improved_phaseh_refined_combo_allocator",
]
FIXED_COMPARATOR_SET = [
    PRODUCTION_PIN,
    LEGACY_SHADOW,
    PRIOR_BOUNDED_REFERENCE,
    REFERENCE_PANEL_VERSION,
    ACTIVE_PANEL_BASELINE,
]
ALL_VERSIONS = FIXED_COMPARATOR_SET + PHASE_H_CANDIDATES
...
def main() -> None:
    ...
    bounded_full = full_df.set_index("version_name").loc[PRIOR_BOUNDED_REFERENCE]
    ...
    bounded_holdout_returns = returns_map[PRIOR_BOUNDED_REFERENCE].tail(HOLDOUT_WEEKS)
    ...
    for name in ALL_VERSIONS:
        ...
        pairwise_row = {
            ...
            "full_raw_delta_vs_prior_bounded_reference": float(candidate_full["raw_target_composite"] - bounded_full["raw_target_composite"]),
            "full_raw_delta_vs_refined_blend": float(candidate_full["raw_target_composite"] - refined_blend_full["raw_target_composite"]),
            "bootstrap_prob_vs_prior_bounded_reference": moving_block_bootstrap_prob(candidate_holdout_returns - bounded_holdout_returns),
    ...
    full_df.to_csv(LAYER3_DIR / "phase_h_candidate_metrics_full.csv", index=False)
    dev_df.to_csv(LAYER3_DIR / "phase_h_candidate_metrics_dev.csv", index=False)
    holdout_df.to_csv(LAYER3_DIR / "phase_h_candidate_metrics_holdout.csv", index=False)
    rolling_df.to_csv(LAYER3_DIR / "phase_h_rolling_origin_summary.csv", index=False)
    pairwise_df.to_csv(LAYER3_DIR / "phase_h_pairwise_validation.csv", index=False)

    protocol = {
        "phase": "Phase H",
        "reference_panel": REFERENCE_PANEL_VERSION,
    ...
    (LAYER3_DIR / "phase_h_validation_protocol.json").write_text(json.dumps(protocol, indent=2))

    print("Saved Phase H evaluation artifacts:")
    for name in [
        "data/05_layer3_portfolio_construction/phase_h_candidate_metrics_full.csv",
        "data/05_layer3_portfolio_construction/phase_h_candidate_metrics_dev.csv",
        "data/05_layer3_portfolio_construction/phase_h_candidate_metrics_holdout.csv",
        "data/05_layer3_portfolio_construction/phase_h_rolling_origin_summary.csv",
        "data/05_layer3_portfolio_construction/phase_h_pairwise_validation.csv",
        "data/05_layer3_portfolio_construction/phase_h_validation_protocol.json",
    ]:
        print(" -", name)
